...
Stmt ScriptCompleter::VisitStmt_(const BlockRealizeNode* op) {
  contains_block = true;
  for (const PrimExpr& value : op->iter_values) {
    CHECK(value.dtype().is_int()) << "BlockRealize iter_value expected a IntImm, but got "
                                  << value.dtype();
  }
  return StmtMutator::VisitStmt_(op);
}

Stmt ScriptCompleter::VisitStmt_(const BlockNode* op) {
