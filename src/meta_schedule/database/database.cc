...
Database DatabaseNode::DumpPruned(const String& path_workload, const String& path_tuning_record) {
  ...
  Database db = Database::JSONDatabase(path_workload, path_tuning_record, true);
  db->mod_eq_ = this->mod_eq_->Clone();
...
TVM_REGISTER_GLOBAL("meta_schedule.DatabasePruneAndDump")
