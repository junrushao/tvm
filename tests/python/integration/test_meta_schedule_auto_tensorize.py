...
def tune_and_test(relay_mod, data_np, weight_np, op_name, target, sch_rules, postprocs):
    ...
    tune_tasks = list(
        filter(
            ...
            ms.relay_integration.extract_task_from_relay(relay_mod, target, params),
        )
    )
    with tempfile.TemporaryDirectory() as work_dir:
        tasks, task_weights = ms.relay_integration.extract_tasks_to_tune_context(
...
def _test_bert_int8(relay_mod, params, input_info, target, sch_rules, postprocs):
    ...
    tune_tasks = [
        task
        for task in ms.relay_integration.extract_task_from_relay(relay_mod, target, params)
