...
def extract_task_from_relay(
    ...
    params: Optional[Dict[str, NDArray]],
    *,
    tir_converter: Literal["default", "allow_extern"] = "default",
) -> List[ExtractedTask]:
...
def extract_tasks_to_tune_context(
    ...
    for task, logger, rand_state in zip(
        extracted_tasks,
        get_loggers_from_work_dir(work_dir, [t.task_name for t in tasks]),
        fork_seed(seed, n=len(tasks)),
    ):
        tasks.append(
            TuneContext(
...
def tune_relay(
    ...
    *,
    ...
    """
    tasks, task_weights = extract_tasks_to_tune_context(
