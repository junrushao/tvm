...
def _spec_from_torch(
    module: Module,
    name: str,
    args: List[Any],
):
    def _convert_torch(arg_name, arg):
        if isinstance(arg, torch.Tensor):
            _, dtype = str(arg.dtype).rsplit(".", maxsplit=1)
            return _tensor_placeholder(name=arg_name, shape=arg.shape, dtype=dtype)
        if isinstance(arg, int):
            return tir.Var(arg_name, dtype="int64")
        raise TypeError(f"Unsupported argument type: {type(arg)}")

    callable_method = getattr(module, name)
    arg_names = list(inspect.signature(callable_method).parameters.keys())
    if len(arg_names) != len(args):
        raise TypeError(f"Expected {len(arg_names)} arguments, but got {len(args)} arguments")

    converted_args = []
    for arg_name, arg in zip(arg_names, args):
        converted_args.append(_convert_torch(arg_name, arg))

    return _spec.Method(name=name, args=converted_args, method=callable_method)
