...
class RotaryEmbedding(Module):
    ...
    def forward(self, q: Tensor, k: Tensor, offset: tir.Var):
        ...
        q_embed = self.tensor_expr_op(
            rotary_embedding,
            "rotary_embedding",
            args=[q, cos, sin, offset],
        )
        k_embed = self.tensor_expr_op(
            rotary_embedding,
            "rotary_embedding",
            args=[k, cos, sin, offset],
        )
...
class LlamaAttention(Module):  # pylint: disable=too-many-instance-attributes
    ...
    def forward(self, hidden_states: Tensor, attention_mask: Tensor, total_seq_len: tir.Var):
        ...
        h, d = self.num_attention_heads, self.hidden_size // self.num_attention_heads
        ...
        k = reshape(self.k_cache.view(total_seq_len), (t, b, h, d))
        v = reshape(self.v_cache.view(total_seq_len), (t, b, h, d))
...
class LlamaForCasualLM(Module):
    ...
    def forward(self, inputs: Tensor, total_seq_len: tir.Var, attention_mask: Tensor):
        ...
        hidden_states = self.tensor_expr_op(
            _index,
            name_hint="index",
            args=[hidden_states],
        )
    ...
    def prefill(self, inputs: Tensor, total_seq_len: tir.Var):
        ...
        attention_mask = self.tensor_expr_op(
            _attention_mask,
